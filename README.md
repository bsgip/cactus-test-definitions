...
| `response-contents` | `latest: bool/None` `status: int/None` `all: bool/None` `time_tariff_interval: bool/None` `exists: bool/None` | True if at least one received Response matches the filter. `latest` will only consider the most recent received Response. `all` will look for a nominated status match for every `DERControl`. `time_tariff_interval` will swap from matching `DERControlResponse` to `PriceResponse` and `exists` can force the check to assert that the `Response` is missing. |
