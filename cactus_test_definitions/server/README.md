...
| `upsert-mup` | `mup_id: str` `location: CSIPAusReadingLocation` `reading_types: list[CSIPAusReadingType]` `mmr_mrids: list[str]/None` `pow10_multiplier: int/None`  `expect_rejection: bool/None` `set_mup_mrid: str/None` | Submits a MirrorUsagePoint with MirrorMeterReading's. Will ensure stable MRID values for the same sets of parameters (unless overridden with mmr_mrids). `mup_id` will alias this MirrorUsagePoint for future action calls. |
...
| `upsert-der-capability` | `type: int` `rtgMaxW: int` `modesSupported: int` `doeModesSupported: int` `rtgMaxVA: int/None` `rtgMaxVar: int/None` `rtgMaxVarNeg: int/None` `rtgMinPFOverExcited: int/None` `rtgMinPFUnderExcited: int/None` `rtgMaxChargeRateW: int/None` `rtgMaxDischargeRateW: int/None` `rtgMaxWh: int/None` `vppModesSupported: int/None` | Sends DERCapability - validates that the server persisted the values correctly. `rtgMinPFOverExcited`/`rtgMinPFUnderExcited` are PowerFactor displacement values scaled by 10^-2 (e.g. `90` = PF 0.90) |
| `upsert-der-settings` | `setMaxW: int` `setGradW: int` `modesEnabled: int` `doeModesEnabled: int` `setMaxVA: int/None` `setMaxVar: int/None` `setMaxVarNeg: int/None` `setMinPFOverExcited: int/None` `setMinPFUnderExcited: int/None` `setMaxChargeRateW: int/None` `setMaxDischargeRateW: int/None` `setMaxWh: int/None` `setMinWh: int/None` `vppModesEnabled: int/None` | Sends DERSettings - validates that the server persisted the values correctly. `setMinPFOverExcited`/`setMinPFUnderExcited` are PowerFactor displacement values scaled by 10^-2 (e.g. `90` = PF 0.90) |
...
| `der-control` | `minimum_count: int/None` `maximum_count: int/None` `latest: bool/None` `opModImpLimW: float/None` `opModExpLimW: float/None` `opModLoadLimW: float/None` `opModGenLimW: float/None` `opModEnergize: bool/None` `opModConnect: bool/None` `opModFixedW: float/None` `opModStorageTargetW: float/None` `rampTms: int/None` `randomizeStart: int/None` `event_status: int/None` `responseRequired: int/None` `derp_primacy: int/None` `sub_id: str/None` | Are there enough DERProgram(s) that satisfy the filter criteria? `latest` will ONLY match the most recent DERControl. |
