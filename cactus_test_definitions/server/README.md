...
| `refresh-resource` | `resource: CSIPAusResource` `expect_rejection: bool/None` | Forces a particular resource to be refreshed (using existing hrefs in context). Can be set to expect a HTTP 4XX ErrorResponse. |
